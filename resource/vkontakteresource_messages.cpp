...
void VkontakteResource::messageListFetched(KJob *kjob)
{
    Q_ASSERT(!m_idle);
    Vkontakte::AllMessagesListJob * const job = dynamic_cast<Vkontakte::AllMessagesListJob*>(kjob);
    Q_ASSERT(job);
    m_currentJobs.removeAll(job);

    if (job->error())
    {
        abortWithError(i18n("Unable to get messages from server: %1", job->errorString()),
...
void VkontakteResource::messageListUsersFetched(KJob *kjob)
{
    Q_ASSERT(!m_idle);
    ...
    if (job->error())
    {
        abortWithError(i18n( "Unable to get users for message list from server: %1", job->errorString()),
                       job->error() == Vkontakte::VkontakteJob::AuthenticationProblem);
        return;
    }

    // TODO: UserInfoJob::userInfoMap
    QList<Vkontakte::UserInfoPtr> usersList = job->userInfo();
...
void VkontakteResource::messageDiscussionsFetched(KJob *kjob)
{
    Q_ASSERT(!m_idle);
    Vkontakte::DiscussionsListJob * const job = dynamic_cast<Vkontakte::DiscussionsListJob*>(kjob);
    Q_ASSERT(job);
    m_currentJobs.removeAll(job);

    if (job->error())
    {
        abortWithError(i18n("Unable to get discussion list from server: %1", job->errorString()),
                       job->error() == Vkontakte::VkontakteJob::AuthenticationProblem);
        return;
    }

    QSet<int> discussionIds;
    foreach (const Vkontakte::MessageInfoPtr &message, job->list())
